...
Production application and backfill are intentionally outside #169. Before a future deployment, take the standard SQLite backup and run the migration against a disposable restored copy. Removal is a forward migration, never a rewritten `0066`: drop `body_check_in_measurements`, then `body_check_ins`, then `body_check_in_preferences`. That removal deletes only the new domain; it does not touch scalar `body_measurements`. If data preservation is required, export the three new tables before the forward removal migration.
