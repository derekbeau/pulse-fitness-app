...
`dailyContextRuntimeResponseSchema.journalObservations` is an ordered owner-scoped array for the requested local date. `observations` remains the flare array. Check-in source-kind and identity rules are unchanged. Journal write sources reuse the #179 current-token resolver. Journal receipts replay verified historical snapshots without revalidating mutable linked sources; each new create/correction validates current owned tokens. Weekly facts allow up to 101 links so a Journal observation with 100 source links can carry its own current revision link. The Journal list labels old rows `legacy_date_only` and states their missing provenance.
