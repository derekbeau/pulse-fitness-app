...
## Final adaptive ordering regression for independent delta

- The actual adaptive store integration test now creates a baseline and manual check-in through normal preview and acceptance at an identical millisecond with IDs that sort opposite their acceptance order. It asserts latest accepted state on both SQLite connections, unchanged consumed-window behavior, exact two-event target history, and idempotent acceptance replay. A one-millisecond later control covers ordinary timestamp ordering. With the prior UUID tie-break restored temporarily, only the equal-time case failed; the rowid query passed both cases. Raw red/green and focused receipts are in `checkpoint-final-adaptive-regression`.
- One pending check-in per program and immediate transactional acceptance mean the test's two accepted rows are inserted and accepted sequentially. Rowid breaks equal-time ties by insertion order; it is not a general acceptance-event sequence if an older pending row could be accepted after a newer one through a different future import path. No such reachable residual defect was established in the current workflow.
- The owned API restart probe established exact decoded API identity equality for the measurement, workout, and target. Chromium verified the Body date, workout name, and 2,500-calorie target were visible before and after restart; it did not assert a visual measurement ID or weighted set values. The separate repeated-occurrence Chromium flow covers set values and correction.
- Bounded consumer browser coverage is complete under the parent brief. This narrow test is for independent delta review; it does not add a broad browser rerun, release acceptance, merge, deployment, production action, or issue closure.
...
- Workout browser coverage now completes the current configured feedback, verifies persisted completed sets, and corrects one of two repeated exercise occurrences without changing the other. Active-workout hydration rechecks occurrence structure when template data arrives after session data; otherwise a hard reload could show 0/0 cards despite persisted sets. The owned API restart preserves exact measurement, workout, and target identities on API readback, with bounded browser visibility described above.
- This is a bounded final browser repair, with no claim of a fresh security, concurrency, advanced trend, or production recertification. The narrow adaptive regression remains for independent delta review and parent release disposition. No merge, deployment, production mutation, or issue closure occurred.
