...
Status: approved, implementation-ready backend handoff. This task is documentation-only: do not launch an agent, implement code, run a server, execute migrations against live data, deploy, merge, publish a PR, or do UI, media, analytics, or photo work.
...
Base reference: `main` at `7e64f6782fc27f504bcf06c9dfd4b9547ca78ea0`, verified against `origin/main` before authoring. The implementation agent must start from a clean branch based on the current approved base and must record its actual starting HEAD; do not substitute this base reference or a stale prepared commit. Runtime preference if launched later: primary GPT-5.6 Sol, medium, Fast OFF; internal/review GPT-5.6 Luna, medium, Fast OFF. These are launcher settings, not acceptance gates.
...
Use one isolated backend lane; no simultaneous editor and no changes to `/Users/meridian/Projects/pulse-body-progress-ui`. Implement the full contract without reducing the #124 scope. Use focused tests during implementation, including synthetic fresh/populated-legacy migrations, rollback/removal documentation, raw-reading/quality tests including equal closest pairs, preference/due, draft/resume/completion, auth/ownership/concurrency/idempotency/deletion/export/privacy/context, and existing scalar CRUD compatibility (same-date merge, null omission, unit conversion). Verify genuine API/migration behavior, raw source evidence, and source/config/dependency binding. Then run ONE final uncached full gate: `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build` plus relevant API/OpenAPI checks. Capture the first-run raw receipt, actual starting and final clean SHAs, source/config/dependency lock identity, and test bindings. Backend only: no browser ritual and no UI acceptance claim. After completion, draft a PR for review, but do not merge, deploy, backfill, execute against live DB, or publish/ship anything. Do not launch, change config/env, or touch production.
...
The existing worktree `/Users/meridian/Projects/pulse-body-progress-ui` at `f24b70033dd18c343db0e83c440f4a40047687b4` remains outside this backend lane and must not be edited, deleted, reset, or launched. This is an external coordination note only. The implementation agent must refuse the reduced #124 contract and use this live-issue-derived bridge. Deployment remains on hold until Whole Body Progress is ready; this handoff does not lift that hold.

The equal-closest-pairs policy is approved: if closest pairs are exactly tied, choose the earliest pair in reading order `(reading1,reading2)`, then `(reading1,reading3)`, then `(reading2,reading3)`. No other product question is open. Implementation may proceed from the complete contract when separately launched/authorized. Paste-ready manual prompt: “Read live #120/#121/#124 and `docs/implementation/body-check-in-foundation-goal.md` at the exact base SHA. Implement only the backend completion bridge on an isolated branch. Preserve existing CRUD compatibility and historical scalar facts; add strict preferences, check-ins, repeated readings, server quality, due states, context, and safety contracts. Resolve the documented equal-pair decision exactly as approved. Run the required synthetic migration/API/compatibility tests and uncached full gates. Do not touch UI/media/analytics/photos, production, deployment, backfill, merge, or PR publication. Return exact SHA and raw receipts.”
