...
The backend does not claim to understand arbitrary prose. An AgentToken supplies a declared bounded `semanticTopic`, and the database hashes the authenticated subject, local date, timezone, normalized declared topic, sorted owned current source references (including their current revision where applicable), and optional parent question. Prompt punctuation, thread identity, and client idempotency key are intentionally excluded. Thus retries and cross-thread paraphrases of the same declared topic/source/day resume one question, while changed source revisions, a different source, a different day, or a legitimate new topic produce a distinct question. Follow-ups must name an owned parent question and retain both parent and source links. An answered question remains answered under its canonical identity; a new prompt alone cannot requeue it.
