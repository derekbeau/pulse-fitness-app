CREATE TABLE `daily_check_in_questions` (
  ...
  `follow_up_question_id` text REFERENCES `daily_check_in_questions`(`id`) ON DELETE SET NULL,
...
CREATE TABLE `daily_check_in_question_revisions` (
  ...
  `prior_revision_id` text REFERENCES `daily_check_in_question_revisions`(`id`) ON DELETE CASCADE,
...
CREATE TABLE `daily_check_in_answer_revisions` (
  ...
  `question_id` text NOT NULL REFERENCES `daily_check_in_questions`(`id`) ON DELETE CASCADE,
  `question_revision_id` text NOT NULL REFERENCES `daily_check_in_question_revisions`(`id`) ON DELETE CASCADE,
  `user_id` text NOT NULL REFERENCES `users`(`id`) ON DELETE CASCADE,
  `revision` integer NOT NULL,
  `prior_revision_id` text REFERENCES `daily_check_in_answer_revisions`(`id`) ON DELETE CASCADE,
  `state` text NOT NULL,
  `value` text,
  `source_json` text NOT NULL,
  `answered_at` text NOT NULL,
